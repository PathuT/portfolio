          ...
          >
            ...
            <div className="space-y-4">
              {portfolioData.education.map((edu, index) => (
                <motion.div
                  key={index}
                  className="bg-white/[0.03] backdrop-blur-sm border border-white/10 hover:border-white/20 p-6 rounded-2xl transition-colors duration-300"
            ...
            <div className="space-y-3">
              {portfolioData.certifications.map((cert, index) => {
                ...
                return (
                  <motion.div
                    key={index}
                    className={`bg-white/[0.03] backdrop-blur-sm border border-white/10 hover:border-white/20 p-5 rounded-2xl border-l-4 transition-colors duration-300`}
                    ...
                    transition={{ duration: 0.5, delay: index * 0.1 }}
                  >
                    <h4 className="font-semibold text-white mb-1 text-sm leading-snug">{cert.title}</h4>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {portfolioData.achievements.map((achievement, index) => {
              ...
              return (
                <motion.div
                  key={index}
                  className="bg-white/[0.03] backdrop-blur-sm border border-white/10 hover:border-white/20 p-6 rounded-2xl text-center transition-all duration-300"
                ...
                >
                  ...
                  <h4 className="font-bold text-white mb-1.5 text-sm leading-snug">{achievement.title}</h4>
                  <p className="text-slate-400 text-xs">{achievement.organization}</p>
                </motion.div>
              );
            })}
          </div>
